...
const gfe4x Gk = {
    {{repeat4x(3338585.0)},
     ...
     {repeat4x(
         13159058716893486699394031679446200360393917757201178927420145664.0)},
     {repeat4x(
         12842070454865951878207543570322902610654944894655310136406629955928064.0)},
     {repeat4x(
         16295354408597167049195255459117446390458785936524946835293367493552880222208.0)}}};

extern void ge4x_niels_add_p1p1_asm(ge4x_p1p1 *, const ge4x *,
                                    const ge4x_niels *);
...
extern void ge4x_lookup_niels_asm(ge4x_niels *, const double (*)[3][12],
                                  const char *);
...
void ge4x_scalarsmults_base(ge4x *a, const sc25519 *s) {
    ...
    for (j = dist; j < 64; j += dist) {
        for (i = 0; i < dist; i++) {
            ...
            ge4x_lookup_niels_asm(&tmp[i], ge4x_base_multiples_niels[j / dist],
                                  idx);
        }
...
void ge4x_scalarsmults_table(ge4x *a, ge4x (*table)[8], const sc25519 *s,
                             int dist) {
    int i, j, pos;
    ge4x t;
    ge4x_p1p1 t_p1p1;
...
void ge4x_hash(unsigned char *k, unsigned char *sp, unsigned char *q, ge4x *p) {
    int i, j;
